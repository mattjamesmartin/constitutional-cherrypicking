...
match_count_label = widgets.Label(value=f"Total Matches: {len(results)}") # Widget for displaying match count
...
def perform_ngram_search():
    ...
    and sort results by segment ID structure.
    """
    global results
    results = []
    if ngram:  # Only search if ngram is not empty
        for i, (segment_id, segment) in enumerate(sorted_segments):
            if ngram.lower() in segment.get('text', '').lower():  # Check if n-gram is in text
                # Extract the document ID from the segment ID
                document_id = '/'.join(segment_id.split('/')[:-1])

                # Retrieve metadata from documents_dict
                metadata = model_dict['documents_dict'].get(document_id, {})
                results.append((
                    segment_id,
                    handle_missing(segment.get('text', '')),  # Text
                    handle_missing(metadata.get('seat', '')),  # Seat
                    handle_missing(metadata.get('full_name', '')),  # Full Name
                    handle_missing(metadata.get('municipality', '')),  # District
                    handle_missing(metadata.get('province', '')),  # Region
                    handle_missing(metadata.get('vote_perc', '')),  # Vote Percentage
                    handle_missing(metadata.get('sex', '')),  # Sex
                    handle_missing(metadata.get('age_feb2018', '')),  # Age
                    handle_missing(metadata.get('occupation', '')),  # Occupation
                    handle_missing(metadata.get('educ_level', '')),  # Education level
                    handle_missing(metadata.get('PCC', '')),  # PCC
                    handle_missing(metadata.get('CDR', '')),  # CDR
                    handle_missing(metadata.get('CTC', '')),  # CTC
                    handle_missing(metadata.get('FMC', '')),  # FMC
                    handle_missing(metadata.get('ANAP', '')),  # ANAP
                    handle_missing(metadata.get('ACRC', '')),  # ACRC
                    handle_missing(metadata.get('FEU', '')),  # FEU
                    handle_missing(metadata.get('UJC', '')),  # UJC
                    handle_missing(metadata.get('central_committee', '')),  # Central Committee
                    handle_missing(metadata.get('politburo', '')),  # Politburo
                    handle_missing(metadata.get('draft_commission', '')),  # Draft Commission
                ))

    # Sort the results based on the numeric structure of segment IDs
    results.sort(key=lambda x: parse_segment_id(x[0]))

    recalculate_context()
    match_count_label.value = f"Total Matches: {len(results)}"  # Update match count
...
def export_with_tags_and_metadata():
    ...
    # Write the header
    sheet.append([
        'segment_ID', 'text', 'tags', 'seat', 'full_name', 'municipality', 'province',
        'vote_perc', 'sex', 'age_feb2018', 'occupation', 'educ_level', 'PCC', 'CDR',
        'CTC', 'FMC', 'ANAP', 'ACRC', 'FEU', 'UJC', 'central_committee',
        'politburo', 'draft_commission'
    ])
    ...
    for result in results:
       ...
       # Metadata fields
        metadata = [
            result[2],  # Seat
            result[3],  # Full Name
            result[4],  # Municipality
            result[5],  # Province
            result[6],  # Vote Percentage
            result[7],  # Sex
            result[8],  # Age
            result[9],  # Occupation
            result[10], # Education Level
            result[11], # PCC
            result[12], # CDR
            result[13], # CTC
            result[14], # FMC
            ...
            result[17], # FEU
            result[18], # UJC
            result[19], # Central Committee
            result[20], # Politburo
            result[21]  # Draft Commission
        ]
        ...
        if toggle_states[segment_id]:  # If toggled on, display the context
            ...
            for idx, (context_id, context_segment) in enumerate(results_with_context[segment_id]):
                ...
                context_metadata_content = widgets.VBox([
                    widgets.HTML(f"<p>Seat: {handle_missing(context_metadata.get('seat', ''))}</p>"),
                    widgets.HTML(f"<p>Full Name: {handle_missing(context_metadata.get('full_name', ''))}</p>"),
                    widgets.HTML(f"<p>Municipality: {handle_missing(context_metadata.get('municipality', ''))}</p>"),
                    widgets.HTML(f"<p>Province: {handle_missing(context_metadata.get('province', ''))}</p>"),
                    widgets.HTML(f"<p>Vote Percentage: {handle_missing(context_metadata.get('vote_perc', ''))}</p>"),
                    widgets.HTML(f"<p>Sex: {handle_missing(context_metadata.get('sex', ''))}</p>"),
                    widgets.HTML(f"<p>Age: {handle_missing(context_metadata.get('age_feb2018', ''))}</p>"),
                    widgets.HTML(f"<p>Occupation: {handle_missing(context_metadata.get('occupation', ''))}</p>"),
                    widgets.HTML(f"<p>Education Level: {handle_missing(context_metadata.get('educ_level', ''))}</p>"),
                    widgets.HTML(f"<p>PCC: {handle_missing(context_metadata.get('PCC', ''))}</p>"),
                    widgets.HTML(f"<p>CDR: {handle_missing(context_metadata.get('CDR', ''))}</p>"),
                    widgets.HTML(f"<p>CTC: {handle_missing(context_metadata.get('CTC', ''))}</p>"),
                    widgets.HTML(f"<p>FMC: {handle_missing(context_metadata.get('FMC', ''))}</p>"),
                    widgets.HTML(f"<p>ANAP: {handle_missing(context_metadata.get('ANAP', ''))}</p>"),
                    widgets.HTML(f"<p>ACRC: {handle_missing(context_metadata.get('ACRC', ''))}</p>"),
                    widgets.HTML(f"<p>FEU: {handle_missing(context_metadata.get('FEU', ''))}</p>"),
                    widgets.HTML(f"<p>UJC: {handle_missing(context_metadata.get('UJC', ''))}</p>"),
                    widgets.HTML(f"<p>Central Committee: {handle_missing(context_metadata.get('central_committee', ''))}</p>"),
                    widgets.HTML(f"<p>Politburo: {handle_missing(context_metadata.get('politburo', ''))}</p>"),
                    widgets.HTML(f"<p>Draft Commission: {handle_missing(context_metadata.get('draft_commission', ''))}</p>")
                ])
