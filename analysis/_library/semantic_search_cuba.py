...
def run_search(choice_dict, model_dict, encoder, reviewed_segment_ids):
    ...
    for segment_id, score in results:
        ...
        enriched_results.append((
            ...
            handle_missing(metadata.get("seat", "")),  # Seat number
            handle_missing(metadata.get("full_name", "")),  # Speaker
            handle_missing(metadata.get("municipality", "")),  # Municipality
            handle_missing(metadata.get("province", "")),  # Province
            ...
            handle_missing(metadata.get("CDR", "")),
            handle_missing(metadata.get("CTC", "")),
            handle_missing(metadata.get("FMC", "")),
            handle_missing(metadata.get("ANAP", "")),
            handle_missing(metadata.get("ACRC", "")),
            handle_missing(metadata.get("FEU", "")),
            handle_missing(metadata.get("UJC", "")),
            handle_missing(metadata.get("central_committee", "")),  # Central Committee
            handle_missing(metadata.get("politburo", "")),  # Politburo
            handle_missing(metadata.get("draft_commission", ""))  # Drafting commission
        ))
                ...
                if toggle_state["is_open"]:
                    ...
                    for idx, (ctx_id, ctx_text, is_match) in enumerate(
                        ...
                        context_metadata_content = widgets.VBox([
                            widgets.HTML(f"<p>Seat: {handle_missing(ctx_metadata.get('seat', ''))}</p>"),
                            widgets.HTML(f"<p>Full Name: {handle_missing(ctx_metadata.get('full_name', ''))}</p>"),
                            widgets.HTML(f"<p>Municipality: {handle_missing(ctx_metadata.get('municipality', ''))}</p>"),
                            widgets.HTML(f"<p>Province: {handle_missing(ctx_metadata.get('province', ''))}</p>"),
                            widgets.HTML(f"<p>Vote Percentage: {handle_missing(ctx_metadata.get('vote_perc', ''))}</p>"),
                            ...
                            widgets.HTML(f"<p>Education Level: {handle_missing(ctx_metadata.get('educ_level', ''))}</p>"),
                            widgets.HTML(f"<p>PCC: {handle_missing(ctx_metadata.get('PCC', ''))}</p>"),
                            widgets.HTML(f"<p>CDR: {handle_missing(ctx_metadata.get('CDR', ''))}</p>"),
                            widgets.HTML(f"<p>CTC: {handle_missing(ctx_metadata.get('CTC', ''))}</p>"),
                            widgets.HTML(f"<p>FMC: {handle_missing(ctx_metadata.get('FMC', ''))}</p>"),
                            widgets.HTML(f"<p>ANAP: {handle_missing(ctx_metadata.get('ANAP', ''))}</p>"),
                            widgets.HTML(f"<p>ACRC: {handle_missing(ctx_metadata.get('ACRC', ''))}</p>"),
                            widgets.HTML(f"<p>FEU: {handle_missing(ctx_metadata.get('FEU', ''))}</p>"),
                            widgets.HTML(f"<p>UJC: {handle_missing(ctx_metadata.get('UJC', ''))}</p>"),
                            widgets.HTML(f"<p>Central Committee: {handle_missing(ctx_metadata.get('central_committee', ''))}</p>"),
                            widgets.HTML(f"<p>Politburo: {handle_missing(ctx_metadata.get('politburo', ''))}</p>"),
                            widgets.HTML(f"<p>Draft Commission: {handle_missing(ctx_metadata.get('draft_commission', ''))}</p>")
                        ])
...
def export_with_tags_and_metadata(cluster_dict):
    ...
    # Write the header row, now including the new metadata fields
    sheet.append([
        'Cluster', 'segment_ID', 'text', 'semantic_score', 'tags', 'seat', 'full_name',
        'municipality', 'province', 'vote_perc', 'sex', 'age_feb2018', 'occupation',
        'educ_level', 'PCC', 'CDR', 'CTC', 'FMC', 'ANAP', 'ACRC', 'FEU', 'UJC',
...
def append_segment_to_sheet(sheet, cluster_name, segment_data):
    ...
    # Metadata fields (updated to match the new structure)
    metadata = [
        segment_data[3],  # Seat
        segment_data[4],  # Full Name
        segment_data[5],  # Municipality
        segment_data[6],  # Province
        segment_data[7],  # Vote Percentage
        segment_data[8],  # Sex
        segment_data[9],  # Age
        segment_data[10],  # Occupation
        segment_data[11],  # Education Level
        segment_data[12],  # PCC
        segment_data[13],  # CDR
        segment_data[14],  # CTC
        segment_data[15],  # FMC
        segment_data[16],  # ANAP
        segment_data[17],  # ACRC
        segment_data[18],  # FEU
        segment_data[19],  # UJC
        segment_data[20],  # Central Committee
        segment_data[21],  # Politburo
        segment_data[22],  # Draft Commission
    ]
